...
└────────┬────────────────┘
         ...
         ▼
┌──────────────────────────┐
│  Beautiful Login Page    │◄─────── Served by your backend
│  (Material-UI Design)    │         Built from server-ui/
│  • Sign in with SSO      │
│  • Direct API Key Option │
└────────┬─────────────────┘
         │
         │ 3. User clicks "Sign in with SSO"
         │    Redirects to YOUR OAuth Provider
         ▼
┌─────────────────────────┐
│  Your OAuth Provider    │
│  (e.g., Okta, Azure AD, │
...
│  (Shows loading...)     │         Built from server-ui/
└────────┬────────────────┘
         │
         │ 6. POST /auth/oauth2/callback
...
└────────┬────────────────┘
         ...
         │    Call: YOUR_OAUTH_TOKEN_ENDPOINT
...
└────────┬────────────────┘
         ...
         │    Call: YOUR_OAUTH_USERINFO_ENDPOINT
...
└────────┬────────────────┘
         │
         │ 9. Look up user's DQM credentials in YOUR database
         ▼
┌───────────────────────────────────┐
│   Your User Database              │
│                                   │
│   Table: user_dqm_credentials     │
│   ┌──────────┬────────────────┐   │
│   │ user_id  │ dqm_api_key    │   │◄─── ENCRYPTED!
│   │          │ dqm_website_id │   │
│   └──────────┴────────────────┘   │
│                                   │
│   Query:                          │
│   SELECT api_key, website_id      │
│   WHERE user_id = $userId         │
└────────┬──────────────────────────┘
         │
         │ 10. Decrypt API key from database
         │     const apiKey = decrypt(dbRecord.api_key);
         ▼
┌─────────────────────────┐
│  Create Session         │
│  in SessionStore        │
...
│                                 │
│  session:{token}                │
│  ├─ apiKey: "real_crownpeak_…"  │◄─── From YOUR database
│  ├─ websiteId: "site123"        │
│  ├─ userId: "user456"           │
│  └─ expiresAt: 1234567890       │
└────────┬────────────────────────┘
         │
         │ 12. Return session token + websiteId
         │     Response: { sessionToken, websiteId, userId }
         ▼
┌─────────────────────────┐
│  Callback Page          │
│  Receives Token         │
└────────┬────────────────┘
         │
         │ 13. postMessage to parent window
         │     { type: 'DQM_AUTH_SUCCESS', sessionToken, websiteId }
         │ 14. Auto-close popup
         ▼
┌─────────────────────────┐
│  Widget Receives Token  │
...
└────────┬────────────────┘
         │
         │ 15. All DQM API requests now use:
...
└────────┬────────────────┘
         │
         │ 16. Backend extracts session token
         │     Retrieves API key from Redis
         │     Uses real API key to call Crownpeak
         ▼
┌─────────────────────────┐
│  Crownpeak DQM API      │
│  api.crownpeak.net      │
